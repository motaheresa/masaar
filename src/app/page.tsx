"use client";
import { Button } from "@/components/atoms/Button/Button";
import { OutlinePasswordField } from "@/components/molecules/form/Input/PasswordField";
import { OutlineTextField } from "@/components/molecules/form/Input/TextField";
import { IoEyeOffOutline } from "react-icons/io5";

export default function Home() {
  return (
    <div className="flex items-center justify-center h-screen">
      <Button
        onClick={() => console.log("SAd")}
        variant="outline"
      >
        Click Me
      </Button>
      <Button
        onClick={() => console.log("SAd")}
        variant="primary"
        loading={true}
        ProviderImgLink="data:image/png;base64,"
      >
        Click Me
      </Button>
      <Button
        onClick={() => console.log("SAd")}
        variant="outline"
        // OutlineSpanClassName="py-0.5! w-full"
        // className="w-55 mx-auto text-center"
        // loading={true}
        // Icon={IoEyeOffOutline}
      >
        Click Me
      </Button>
      <OutlineTextField
        ICon={
          <svg
            className="w-4 h-4 text-primary"
            aria-hidden="true"
            xmlns="http://www.w3.org/2000/svg"
            width="24"
            height="24"
            fill="none"
            viewBox="0 0 24 24"
          >
            <path
              stroke="currentColor"
              strokeLinecap="round"
              strokeWidth="2"
              d="m3.5 5.5 7.893 6.036a1 1 0 0 0 1.214 0L20.5 5.5M4 19h16a1 1 0 0 0 1-1V6a1 1 0 0 0-1-1H4a1 1 0 0 0-1 1v12a1 1 0 0 0 1 1Z"
            />
          </svg>
        }
        label="Email"
        type="email"
        value="Mo"
        onChange={() => {
          console.log("SA");
        }}
      />
      <OutlinePasswordField
        value="Mosad"
        name="ASD"
        onChange={() => {
          console.log("SA");
        }}
      />
    </div>
  );
}
